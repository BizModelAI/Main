...
export const IncomeProjectionChart: React.FC<IncomeProjectionChartProps> = ({
  ...
  const [projectionData, setProjectionData] = useState<IncomeProjectionData | null>(null);
  ...
  const createUserProfile = (quizData: QuizData): string => {
    return `
      Weekly Time Commitment: ${quizData.weeklyTimeCommitment || 20} hours
      Initial Investment: $${quizData.upfrontInvestment || 1000}
      Tech Skills: ${quizData.techSkillsRating || 3}/5
      Experience Level: ${quizData.experienceLevel || 'beginner'}
      Risk Tolerance: ${quizData.riskComfortLevel || 3}/5
      Self Motivation: ${quizData.selfMotivationLevel || 3}/5
      Income Goal: $${quizData.successIncomeGoal || 5000}/month
      Time to First Income: ${quizData.firstIncomeTimeline || '3-6-months'}
      Learning Style: ${quizData.learningPreference || 'hands-on'}
      Work Style: ${quizData.workCollaborationPreference || 'mostly-solo'}
    `;
  };
  ...
  const getFallbackProjections = (businessId: string): IncomeProjectionData => {
    const baseData: Record<string, IncomeProjectionData> = {
      ...
      'dropshipping': {
        monthlyProjections: [
          { month: 'Month 1', income: -500, cumulativeIncome: -500, milestones: ['Store setup', 'Product research'] },
          { month: 'Month 2', income: -200, cumulativeIncome: -700, milestones: ['First sales', 'Ad testing'] },
          { month: 'Month 3', income: 300, cumulativeIncome: -400, milestones: ['Profitable ads'] },
          { month: 'Month 4', income: 800, cumulativeIncome: 400, milestones: ['Scale advertising'] },
          { month: 'Month 5', income: 1500, cumulativeIncome: 1900, milestones: ['Product expansion'] },
          { month: 'Month 6', income: 2200, cumulativeIncome: 4100, milestones: ['Brand building'] },
        ...
        keyFactors: ['Product selection', 'Marketing skills', 'Ad spend management', 'Customer service'],
        assumptions: ['$1000+ initial investment', 'Effective ad campaigns', 'Market research']
      }
    };
      ...
      return (
        <div className="bg-white p-4 border border-gray-200 rounded-xl shadow-lg">
          ...
          {data.milestones && data.milestones.length > 0 && (
            <div className="mt-2">
              <p className="text-sm font-medium text-gray-600">Key Milestones:</p>
              {data.milestones.map((milestone: string, idx: number) => (
                <p key={idx} className="text-xs text-gray-500">• {milestone}</p>
              ))}
            </div>
          )}
        </div>
  ...
  if (!quizData) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">Complete the quiz to see personalized income projections</p>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-3 text-lg text-gray-600">Generating income projections...</span>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-200">
          ...
          <div className="mt-4 pt-4 border-t border-green-200">
            ...
            <div className="text-lg font-bold text-green-800">{projectionData.averageTimeToProfit}</div>
          </div>
        </div>
      </div>
    </div>
